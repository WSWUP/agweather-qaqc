...
def correction(station, log, var1, var1n, var2, var2n, dates, mon, yr, code):
...
# correct given data using parameters in terminal window
def ini_correction(station, var1, var1n, var2, var2n, dates, mon, yr, code):
    # station - string name of station path for saving correction graphs
    # var1 - values of first variable passed
    # var1n - name of first var passed as string
    # var2 - values of second variable passed
    ...
    # Open ini file, will read relevant correction variables based on code passed
    corr_config = configparser.ConfigParser()
    corr_config.read('config.ini')
    print("\nSystem: Reading in .ini file for correction parameters for variable code %s." % code)

    gen_plots = corr_config['MODES'].getboolean('bokeh_plots')
    int_start = corr_config['CORRECTIONS'].getint('interval_start')
    int_end = corr_config['CORRECTIONS'].getint('interval_end')

    # Check that intervals are defined correctly
    if int_start == -1 and int_end == -1:
        # Full data correction
        int_start = 0
        int_end = var1.shape[0]
    elif int_start != -1 and int_end == -1:
        # Defined start to end of data
        int_end = var1.shape[0]
    else:
        # Fully defined interval
        # Check to see that given end index isn't out of bounds
        if int_end > var1.shape[0]:
            int_end = var1.shape[0]
        else:
            pass
    ...
    # Temperature data
    if code == 1:
        t_f_pass = corr_config['CORRECTIONS'].getint('t_first_pass')
        t_s_pass = corr_config['CORRECTIONS'].getint('t_second_pass')
        ...
        # Conduct second pass
        limit = t_s_pass
        # Recreate average monthly temperatures now that outliers have been removed
        mon_var1 = []
        mon_var2 = []
        var1_std = []
        var2_std = []
        ...
        while k <= 12:
            ...
            var2_std.append(numpy.nanstd(corr_var2[t_index]))
            k += 1
        # Conduct second pass to throw out outliers
        i = int_start
        while i < int_end:
    ...
    # RH data
        ...
        # Corrects the data based on a year-based percentile correction
        corr_thresh = corr_config['CORRECTIONS'].getint('rh_corr_prct')
        print("\n Now correcting RHmax and RHmin data.")
        ...
        # In this case the ini file correction does the full bracket so we do not need to line up the correction
        # array with the actual year of the data array, they are already lined up.
    ...
    # Solar data
        ...
        # Corrects data based on a period-based percentile correction
        discard_thresh = corr_config['CORRECTIONS'].getint('rs_despiking_prct')
        corr_period = corr_config['CORRECTIONS'].getint('rs_period')
        corr_thresh = corr_config['CORRECTIONS'].getint('rs_corr_prct')
        ...
        # now apply those values to the data for each period
        ex_rso_counter = 0  # Counts number of times corrected Rs exceeded Rso and was clipped to Rso
        x = int_start  # index that tracks along data points for full interval
        y = 0  # index that tracks how far along a period we are
        z = 0  # index that tracks which correction period we are in
        while x < len(var1) and x < int_end and z < len(corr_values_var1):
            ...
            if y <= corr_period:
                ...
                y = 1
                z += 1
        print("\n" + str(ex_rso_counter) + " Rs datapoints were clipped to Rso.")

    # Wind or Precipitation
    else:
        # TODO add user defined brackets for wind and precip data
        pass
    ...
    if gen_plots:
        ...
        corrfig = gridplot([[cs1], [cs2], [cs3]], toolbar_location="left")
        save(corrfig)
    else:
        pass

    # return corrected variables
    return corr_var1, corr_var2
